...
import com.bitspan.bitsjobkaro.databinding.FragmentJobAppliedBinding
...
class JobAppliedFragment : Fragment() {
    ...
    private lateinit var appliedJobList: ArrayList<AllJobData>
    private lateinit var adapterr: AllJobAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (shimmerAndDataFlag) {
            getAppliedJob()
            CommonUiFunctions.startShimmer(binding.shimmer, binding.jobAppRecyView)

        } else {

            binding.apply {
                jobAppRecyView.apply {
                    adapter = adapterr
    ...
    private fun getAppliedJob() {
        viewLifecycleOwner.lifecycleScope.launch {
            appliedJobViewModel.getAppliedJob(userId.toInt()).let {
                if (it.isSuccessful && it.body() != null) {
                    CommonUiFunctions.stopShimmer(binding.shimmer, binding.jobAppRecyView)
                    if (it.body()!!.status == 200) {
                        appliedJobList = (it.body()?.data ?: listOf()) as ArrayList<AllJobData>
                        val adapter = AllJobAdapter(appliedJobList) {}
                        binding.jobAppRecyView.adapter = adapter
                    } else {
                        binding.jobAppRecyView.visibility = View.GONE
